...
if (isset($_POST['signup'])) {
	...
	$admin = "Affilation Request on New Site";
	$subject2 = "Affilation Request MinD Webs (mindwebs.org)";
	$subject = "Registration at MinD Webs (mindwebs.org)";
	$content = "\nHere are the details of a new affilation:\n Name: $name \n Email: $email \n Password : $password (Store it in a safe place)\n\n-------------------------------------\nSubmitted Response :\nArea of Interest: $interest\nYour Message : $message\n-------------------------------------\n\n Link : web.mindwebs.org"; 
	...
	if($password != $cpassword) {
		...
		$errormsg = $cpassword_error;
	}
	
	if (!$error) {
		if(mysqli_query($con, "INSERT INTO users(name,email,phone,password,gender,dob,seq_qns,seq_ans,log_up) VALUES('" . $name . "', '" . $email . "', '" . $phone . "', '" . md5($password) . "', '" .$gender . "', '" .$dob . "', '" .$seq_qns . "', '" .$seq_ans . "',now()) ON DUPLICATE KEY UPDATE password='" . md5($password) . "', phone='" . $phone . "', gender='" .$gender . "',dob='" .$dob . "',seq_qns='" .$seq_qns . "',seq_ans='" .$seq_ans . "',log_up=now(),updated_at=now() " )) {
		if(mysqli_query($con, "INSERT INTO admin_mail(user,reciver,subject,content) VALUES('" . $admin . "', '" . $email . "', '" . $subject . "', '" .$content . "')")) {
			...
			} else {	$errormsg = "Error in Requesting...Please try again later! (Quote :Mail Error)";	}
			
		} else {
			$errormsg = "Error in Requesting...Please try again later! (Quote: Session Error)";
		}
	}
}
?>
